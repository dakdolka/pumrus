...
from aiogram.types import WebAppInfo, CallbackQuery, Message, InlineKeyboardMarkup, InlineKeyboardButton
...
async def start(message: Message):
    await message.answer("Открой приложение по кнопке ниже!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Открыть приложение", web_app=WebAppInfo(url=f"{settings.WEB_APP_URL}"))],
    ]))
